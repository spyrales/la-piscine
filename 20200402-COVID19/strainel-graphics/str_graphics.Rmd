...
Certaines séries temporelles peuvent avoir des *irrégularités* liées à la méthode de collecte ; 
il convient alors de les gommer pour mieux faire ressortir la tendance.

```{r datacvs}
covid$deces_cvs <- rep(0,nrow(covid))
lissage <- 7
for (i in (lissage+1):nrow(covid))
  covid$deces_cvs[i] <- mean(covid$deces_j[(i-lissage+1):i])
```
